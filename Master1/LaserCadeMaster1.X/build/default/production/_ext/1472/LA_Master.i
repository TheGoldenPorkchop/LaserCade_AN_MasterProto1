...
# 1 "../MASTER.inc" 1
# 46 "../MASTER.inc"
CURRENT_ADDRESS EQU 0x20 ;I2C ADDRESS CURRENTLY BEING USED
WRITE_DATA EQU 0x21 ;DATA TO BE WRITTEN TO I2C
RECEIVED_BYTE EQU 0x22 ;DATA RECEIVED THROUGH I2C

UART_MODE EQU 0x30 ;UART MODE FLAGS FOR UART ISR
UART_RX_BYTE EQU 0x31 ;DATA RECEIVED BY UART
ENABLE_TARGET EQU 0x32 ;BIT IS SET HIGH WHEN THE TARGET NEEDS TO BE ENABLED
DISABLE_TARGET EQU 0x33 ;BIT IS SET HIGH WHEN THE TARGET NEEDS TO BE DISABLED
INUSE_TARGET EQU 0x34 ;BIT IS SET HIGH WHEN UART HAS PLACED A TARGET IN THE SLOT
SCORE_ADDRESS EQU 0x35 ;STORES I2C ADDRESS OF TARGET WITH A SCORE (USED FOR UARTSENDSCORE SUBROUTINE)
SCORE_PLAYER EQU 0x36 ;PLAYER THAT SCORED (USED FOR UARTSENDSCORE SUBROUTINE)
CHANGE_SLOT EQU 0x37 ;STORES THE TARGET SLOT ADDRESS IS BEING CHANGED FOR

TGT_ONE_ADDRESS EQU 0x041 ;STORES THE I2C ADDRESS OF ENABLED TARGET ONE
TGT_TWO_ADDRESS EQU 0x42 ;STORES THE I2C ADDRESS OF ENABLED TARGET TWO
TGT_THREE_ADDRESS EQU 0x43 ;STORES THE I2C ADDRESS OF ENABLED TARGET THREE
TGT_FOUR_ADDRESS EQU 0x44 ;STORES THE I2C ADDRESS OF ENABLED TARGET FOUR
TGT_FIVE_ADDRESS EQU 0x45 ;STORES THE I2C ADDRESS OF ENABLED TARGET FIVE
TGT_SIX_ADDRESS EQU 0x46 ;STORES THE I2C ADDRESS OF ENABLED TARGET SIX
TGT_SEVEN_ADDRESS EQU 0x47 ;STORES THE I2C ADDRESS OF ENABLED TARGET SEVEN
TGT_EIGHT_ADDRESS EQU 0x48 ;STORES THE I2C ADDRESS OF ENABLED TARGET EIGHT

I2C_ADDRESS_WRITE EQU 0x49 ;FOR THE I2C Write Command
I2C_DATA EQU 0x50 ;FOR THE I2C Write Command
I2C_ADDRESS_READ EQU 0x51 ;FOR THE I2C Read Command
PLAYER_NUMBER EQU 0x52 ;FOR THE I2C Write Command
GREEN EQU 0x53 ;FOR THE I2C Write Command
RED EQU 0x54 ;FOR THE I2C Read Command
BLUE EQU 0x55 ;FOR THE I2C Write Command
CALIBRATE_OUTPUT EQU 0x56 ;FOR THE I2C Write Command
CALIBRATE_POS_H EQU 0x57 ;FOR THE I2C Read Command
CALIBRATE_POS_L EQU 0x58 ;FOR THE I2C Read Command
LED_NUMBER EQU 0x59 ;FOR THE I2C Read Command
I2C_STEPS EQU 0x60 ;WHICH BYTE NUMBER IS GETTING SENT
...
;*** SET OPTION_REG: ****
  ...
  BANKSEL OPTION_REG
  BCF ((OPTION_REG) and 07Fh), 0 ;\ BCF ((OPTION_REG) and 07Fh), 1 ; >>PRESCALER RATIO SET 1:1

  BCF ((OPTION_REG) and 07Fh), 2 ;
  BSF ((OPTION_REG) and 07Fh), 3 ;PRESCALER ASSIGNED TO WDT
  BCF ((OPTION_REG) and 07Fh), 4 ;TMR0 EDGE SET RISING
  BCF ((OPTION_REG) and 07Fh), 5 ;TMR0 CLOCK SOURCE SET TO INTERNAL
  BSF ((OPTION_REG) and 07Fh), 6 ;((PORTB) and 07Fh), 0/INT SET TO RISING EDGE
  BSF OPTION_REG,7 ;WEAK PULLUP ENABLED

;*** SET INTCON REG: ****

  BANKSEL INTCON
  BCF ((INTCON) and 07Fh), 3 ;ENABLE IOC INTERUPT
  BCF ((INTCON) and 07Fh), 4 ;DISABLE INT EXTERNAL INTERUPT
  BSF ((INTCON) and 07Fh), 6 ;ENABLE PERIPHERAL INTERUPTS
  BSF ((INTCON) and 07Fh), 7 ;ENABLE ALL UNMASKED INTERUPTS

;*** SET PIE1-4 REG: *****

  BANKSEL PIE1
  MOVLW 0x28 ;00101000
  MOVWF PIE1 ;DISABLE ALL PIE1 INTERUPTS, EXCEPT ((PIE1) and 07Fh), 3, ((PIE1) and 07Fh), 5
  BANKSEL PIE2
...
;*** BOOT CODE ****
  ...
  BANKSEL INTCON
  BCF INTCON,0
  BCF ((INTCON) and 07Fh), 0 ;CLEAR ((INTCON) and 07Fh), 0
...
;************************************************************************************
  ...
  PSECT code
    ...
    READ_TARGET_STATUS:
     ...
     BTFSC SSP1CON2,6 ;DETERMINE IF THERE IS AN ACK FROM THE SLAVE
     GOTO BAD1
     BSF SSP1CON2,3 ;ENABLE ((SSP1CON2) and 07Fh), 3 TO RECEIVE DATA FROM THE SLAVE
     BANKSEL SSP1STAT
     BTFSS SSP1STAT,0 ;WAIT UNTIL ALL DATA HAS BEEN SHIFTED INTO SSPBUF FROM THE SLAVE
     GOTO $-1
     BANKSEL SSP1BUF ; MOVFW SSP1BUF ;\\READ STATUS OF CURRENT TARGET
 ...
 CHECKR_W: ;Z=1 Not Busy
    ...
    BAD1:
     ...
     BANKSEL SSP1CON2
     ;BSF ((SSP1CON2) and 07Fh), 2 ;GENERATE A STOP CONDITION
     BSF SSP1CON2,2 ;CREATE STOP CONDITION
     BTFSC SSP1CON2,2
     GOTO $-1 ;WAIT FOR STOP CONDITION TO COMPLETE
...
;
  ...
  RETURN
    ...
    UARTTX:
     BANKSEL PIE1
     BCF ((PIE1) and 07Fh), 4 ;DISABLE UART TX INTERRUPT
     BTFSC ((PIE1) and 07Fh), 4
     GOTO $-2
     BANKSEL PORTA
 ...
 TXPLAYER:
    ...
    UARTSENDSCORE:
     ...
     BANKSEL PIE1
     BSF ((PIE1) and 07Fh), 4 ;ENABLE THE UART TRANSMIT INTERRUPT
     RETURN
# 34 "../LA_Master.asm" 2
...
INTERRUPT:
  BANKSEL PIR1
  BTFSC ((PIR1) and 07Fh), 5 ;IF INTERRUPTED BY UART RECEIVE, GOTO UARTRECEIVE
  GOTO UARTRECEIVE
  BTFSC ((PIR1) and 07Fh), 4 ;IF INTERRUPTED BY UART TRANSMIT, GOTO UARTTRANSMIT
  GOTO UARTTRANSMIT
 ...
 GOBACK:
  ...
  RETFIE ;RETURN TO MAIN, RE-ENABLE ((INTCON) and 07Fh), 7
